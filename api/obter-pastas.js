...
export default async function handler(req, res) {
  ...
  const folderId = "1dfh_85Lg-2LbLtSmyKQ34MuL_RL5aISP";
  ...
  try {
    ...
    const pastas = files.map((file, index) => {
      const tituloLimpo = file.name;

      return {
        id: file.id,
        titulo: tituloLimpo,
        edicao: "Pasta do Drive",
        linkDownload: file.webViewLink,
        icone: "fa-folder-open",
        // Pastas geralmente não tem capa, usaremos o fallback no frontend
        capa: null,
      };
    });

    // Cache agressivo: Retorna a lista instantaneamente da memória da Vercel
    res.setHeader("Cache-Control", "s-maxage=1800, stale-while-revalidate=86400");
    return res.status(200).json(pastas);
  } catch (error) {
    console.error("Erro interno na API do Google Drive (Pastas):", error);
    return res
      .status(500)
